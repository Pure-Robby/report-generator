...
class BarChartSlide extends SlideBase {
    ...
    exportToPPT(pptx) {
        ...
        for (let i = 0; i < labels.length; i++) {
            const item = {
                name: labels[i],
                labels: [this.data.currentLabel || '2024'],
                values: [this.data.current[i]]
            };
            
            if (hasPreviousData) {
                item.labels.push(this.data.previousLabel || '2023');
                item.values.push(this.data.previous[i]);
            }
        ...
        slide.addChart(pptx.ChartType.bar, chartData, {
            ...
            showValue: true,
            valAxisMaxVal: 100,
            valAxisTitleText: 'Percentage',
            catAxisTitleText: 'Categories',
            titleFontSize: 18,
            titleColor: '1e293b'
        });
