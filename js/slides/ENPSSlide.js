class ENPSSlide extends SlideBase {
    ...
    exportToPPT(pptx) {
        ...
        const chartData = [{
            name: this.data.chartData.yearLabel || '2025',
            labels: ['Detractors', 'Passives', 'Promoters'],
            values: [
                this.data.chartData.detractorsPct,
                this.data.chartData.passivesPct,
                this.data.chartData.promotersPct
            ]
        }];
        ...
        slide.addChart(pptx.ChartType.bar, chartData, {
            ...
            barDir: 'bar',
            barGrouping: 'stacked',
            chartColors: ['FE0000', 'FFC000', '00AF50'],
            valAxisMaxVal: 100,
        ...
        this.data.rows.forEach(row => {
            tableRows.push([
                row.name,
                row.n.toString(),
                this.formatScore(row.enpsScore),
                `${row.detractorsPct}%`,
                `${row.passivesPct}%`,
                `${row.promotersPct}%`
            ]);
