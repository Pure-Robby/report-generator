...
class TenPointScaleChartSlide extends SlideBase {
    ...
    exportToPPT(pptx) {
        ...
        // Title
        slide.addText(this.data.title || 'Diversity & Inclusion - Statement Scores', {
            x: 0.5,
            y: 0.5,
            w: 9,
            h: 0.7,
        ...
        slide.addText(this.data.questionText, {
            x: 0.5,
            y: 1.4,
            w: 9,
            h: 0.4,
        ...
        for (let i = 0; i <= 10; i++) {
            ...
            if (i === 0) label = '0 (Not Included)';
            else if (i === 5) label = '5 (Neutral)';
            else if (i === 10) label = '10 (Included)';

            labels.push(label);
            const currentItem = currentDist.find(d => d.rating === i);
            const currentValue = currentItem ? currentItem.percentage : 0;
            currentValues.push(currentValue);
            allPercentages.push(currentValue);
            
            if (hasPrevious) {
                const previousItem = previousDist.find(d => d.rating === i);
                const previousValue = previousItem ? previousItem.percentage : 0;
                previousValues.push(previousValue);
                allPercentages.push(previousValue);
            }
        }
        ...
        // Calculate dynamic y-axis max for PPT chart
        // Round up to next 5% interval, then add one more 5% interval for extra tick
        const maxPercentage = Math.max(...allPercentages, 0);
        const roundedMax = Math.ceil(maxPercentage / 5) * 5;
        const yAxisMax = Math.max(roundedMax + 5, 25);
        ...
        slide.addChart(pptx.ChartType.bar, chartData, {
            ...
            h: 4.0,
            barDir: 'bar',
            barGrouping: hasPrevious ? 'clustered' : 'standard',
            chartColors: hasPrevious
